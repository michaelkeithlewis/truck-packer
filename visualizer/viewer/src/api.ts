...
const CATEGORY_DEFAULT_COLOR = "#9E9E9E";
...
export class CategoryColorScheme implements ColorScheme {
    getPoint(point : Point) : Color {
        return new Color(randomColor());
    }
    getStackable(stackable : Stackable) : Color {
        const cat = extractCategory(stackable.name || "");
        const hex = CATEGORY_COLORS[cat] || CATEGORY_DEFAULT_COLOR;
...
export class StackableRenderer {
    ...
    add(parent: Object3D, colorScheme : ColorScheme, stackPlacement : StackPlacement, x: number, y:number, z: number): Object3D | undefined {
        ...
        if(stackable instanceof Container) {
            ...
            boxGroup.name = boxStackable.name;

            boxGroup.position.x = stackPlacement.y + boxStackable.dy / 2 + x;
            boxGroup.position.y = stackPlacement.z + boxStackable.dz / 2 + y;
            boxGroup.position.z = stackPlacement.x + boxStackable.dx / 2 + z;

            var box = new THREE.Mesh(geometry, material);

            var edgeGeo = new THREE.EdgesGeometry(geometry);
            var edgeMat = new THREE.LineBasicMaterial({ color: 0x222222, linewidth: 2 });
            ...
            box.userData = boxUserData;
            boxGroup.userData = boxUserData;
